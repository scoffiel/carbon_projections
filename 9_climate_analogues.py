...
Purpose: Approach #3
- Match future climate pixels with their present analogue using Mahalanobis distance
- Project future carbon density by assigning that of the present analogue
...
export.to_dataset(name='carbon_change').to_netcdf(root + 'model_output/3_climate_analogues/{}_{}.nc4'.format(SCENARIO,MODEL))

#save table as csv (at least for RCP85 mean scenario) for script 10 plots
#table_cali.to_csv(root + 'model_output/3_climate_analogues/table_rcp85_mean.csv')
...
#plt.savefig(root + 'figures/figS5b_45change.eps')


#Analogue arrows and novelty maps (Fig S7) 
...
ax1.set_title('Climate Analogue Pairs', fontsize=15)
...
cbar.ax.tick_params(labelsize=13)
cbar.set_label('Mahalanobis distance to best analogue', size=13)
...
cbar.set_label('Standardized distance to best analogue', size=13)
...
#plt.savefig(root + 'figures/figS7_arrows-novelty.eps')
...
ax2.set_yticklabels([32,34,36,38,40,42])
ax2.tick_params(top=True, right=True)
#plt.savefig(root + 'figures/figS5a_present.eps')
'''